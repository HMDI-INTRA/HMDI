...
try {
    if ($conn) {
        if (!empty($_POST['solicitacaoTransf']) && !empty($_POST['mesAno'])) {
            ...
            $solicitacaoTransfId = $_POST['solicitacaoTransfId'] ?? null;
            $mesAnoAcao = $_POST['mesAnoAcao'] ?? null;
            ...
            if ($count === 0) {
                $query_inserir = 'INSERT INTO cadastros.analise_dash_indicadores (dt_indicador, analise_indicador, nm_indicador, plano_acao_indicador, nr_tela, dt_fim) VALUES (:dt_indicador, :analise_indicador, :nm_indicador, :plano_acao_indicador, :nr_tela, :dt_fim)';
                $cad_indicador = $conn->prepare($query_inserir);
                $cad_indicador->bindParam(':dt_indicador', $mesAno);
                $cad_indicador->bindParam(':analise_indicador', $analise);
                $cad_indicador->bindParam(':nm_indicador', $solicitacaoTransf);
                $cad_indicador->bindParam(':plano_acao_indicador', $planoAcao);
                $cad_indicador->bindParam(':nr_tela', $solicitacaoTransfId);
                $cad_indicador->bindParam(':dt_fim', $mesAnoAcao);

                if ($cad_indicador->execute()) {
