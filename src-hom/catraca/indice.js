...
class View {
    ...
    renderPaTempoAtendM(data) {
        ...
        function drawVisualization(conteudoArray) {
            var chartData = [
                ...
                ...conteudoArray.map(item => [
                    Utils.formatoMesAno(item.mesAno),
                    parseFloat(item.vermelho), parseFloat(item.vermelho),
                    parseFloat(item.laranja), parseFloat(item.laranja),
                    parseFloat(item.amarelo), parseFloat(item.amarelo),
                    parseFloat(item.verde), parseFloat(item.verde),
                    parseFloat(item.azul), parseFloat(item.azul)
                ]),
            ];
            var data = google.visualization.arrayToDataTable(chartData);
            var options = {
    ...
    renderTaxaMortalidadeFetalView(data) {
        ...
        data.forEach(item => {
            const { MES_ANO, PROCEDIMENTO, FETO_MORTO } = item;
            const key = `${MES_ANO}_${PROCEDIMENTO}`;
            if (!conteudo[key]) {
                conteudo[key] = {
                    mesAno: MES_ANO,
                    procedimento: PROCEDIMENTO,
                    fetoMorto: parseInt(FETO_MORTO, 10) || 0
                };
            } else {
                conteudo[key].fetoMorto += parseInt(FETO_MORTO, 10) || 0;
            }
        });
        ...
        function drawVisualization(conteudoArray) {
            var chartData = [
                ['Mês-Ano e Procedimento', 'Total de Óbitos', { role: 'annotation' }],
                ...conteudoArray.map(item => [
                    `${Utils.formatoMesAno(item.mesAno)} - ${item.procedimento}`,
                    item.fetoMorto, item.fetoMorto
                ]),
            ];
            ...
            var options = {
                title: 'Total de Óbitos fetal',
                vAxis: { title: 'Total de Óbitos' },
                hAxis: { title: 'Mês-Ano e Procedimento' },
                seriesType: 'bars',
                bar: { groupWidth: "80%" },
                annotations: {
                    alwaysOutside: true,
   ...
   renderTaxaMortalidadeNeoView(data) {
        const conteudo = {};
        data.forEach(item => {
            const { MES_ANO, TAXA_MORTALIDADE, TOTAL_OBITOS } = item;
            if (!conteudo[MES_ANO]) {
                conteudo[MES_ANO] = {
                    mesAno: MES_ANO,
        ...
        function drawVisualization(conteudoArray) {
            var chartData = [
                ...
                ...conteudoArray.map(item => [
                    ...
                    parseFloat(item.taxa),
                    `<div style="padding:5px;"><strong>Mês-Ano:</strong> ${Utils.formatoMesAno(item.mesAno)}<br><strong>Taxa de Mortalidade:</strong> ${parseFloat(item.taxa)}<br><strong>Total de Óbitos:</strong> ${item.obitos}</div>`
                ]),
            ];
            var data = google.visualization.arrayToDataTable(chartData);
            var options = {
        ...
        const tableMapaInternacaoJasmin = $('#tableMapaInternacaoJasmin').DataTable({
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                         let observacaoAdulto = '';
                        let dsMedicamentos = '';
                        let dsAlergias = '';
                        let dsTransfusao = '';
                        let ds_procedimento = '';
                        if (row.ds_dieta == 'SMLD') {
                            observacaoAdulto = '';
                            dsMedicamentos = '';
                            dsAlergias = '';
                            dsTransfusao = '';
                            ...
                            observacaoAdulto = Utils.removeImages(row.ds_observacao_aduto).trim();
                            dsMedicamentos = Utils.pularLinhaString(row.ds_medicamentos).trim();
                            dsAlergias = Utils.pularLinhaString(row.ds_alergias).trim();
                            dsTransfusao = Utils.pularLinhaString(row.ds_transfusao).trim();
                            ds_procedimento = Utils.pularLinhaString(row.ds_procedimentos).trim();
                        }
                        return `<div>${observacaoAdulto} <div style='background:#7f3b8a9c;'>${dsMedicamentos}</div> <div style='background:red;'>${dsAlergias}</div> <div style='background:#6d0101;'>${dsTransfusao}</div><div style='background:#1070ffdb;'>${ds_procedimento}</div></div>`;
                    }
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        const dsMedicamentosRn = Utils.pularLinhaString(row.ds_medicamentosRn).trim();
                        const dsAlergiasRn = Utils.pularLinhaString(row.ds_alergiasRn).trim();
                        const dsTransfusaoRn = Utils.pularLinhaString(row.ds_transfusaoRn).trim();
                        const ds_procedimentoRn = Utils.pularLinhaString(row.ds_procedimentosRn).trim();
                        const feto = Utils.pularLinhaString(row.feto).trim();

                        return `<div > 
                                    <div>${observacaoRn}${observacaoAdulto}</div> 
                                    <div>${feto}</div> 
                                    <div style='background:#7f3b8a9c;'>${dsMedicamentosRn}${dsMedicamentos}</div> 
                                    <div style='background:red;'>${dsAlergiasRn}${dsAlergias}</div> 
                                    <div style='background:#6d0101;'>${dsTransfusaoRn}${dsTransfusao}</div> 
                                    <div style='background:#1070ffdb;'>${ds_procedimentoRn}${ds_procedimento}</div>
                                </div>`;
                    }
                },
                {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        return `<div> ${observacaoAdulto}  <div style='background:#7f3b8a9c;'>${dsMedicamentos}</div> <div style='background:red;'>${dsAlergias}</div> <div style='background:#6d0101;'>${dsTransfusao}</div> <div style='background:#1070ffdb;'>${ds_procedimento}</div></d>`;
                    }
                },
                {
                    data: 'ds_observacao_rn', title: 'Observação RN',
