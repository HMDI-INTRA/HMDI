...
if (!empty($_GET['dataInicio']) && !empty($_GET['dataFinal']) && !empty($_GET['ds_periodo'])) {
    ...
    $titulo = "Mapa de Internação Rosa - ";
                        ...
                        <table class='table'>
                            <thead>
                                <tr>
                                    ...
                                    <th scope='col'>Enfermeira</th>
                                    <th scope='col'>Alta</th>
                                </tr>
                            </thead>
                            <tbody>";
                            $sql = "SELECT * FROM cadastros.mapa_internacao_rosa 
        ...
        foreach ($rows as $row) {
            ...
            if($ds_periodo == 'AD'){
                $html .= "<tr>
                            ...
                            <td>{$obsAdulto} <div style='background:#7f3b8a9c;'>{$medicamento}</div> <div style='background:#1070ffdb;'>{$procedimetos}</div>  <div style='background:#ffcd05;'>{$alergias}</div> <div style='background:red;'>{$tranfusao}</div></td>
                            <td>{$obsRn} <div style='background:#7f3b8a9c;'>{$medicamentoRn}</div> <div style='background:#1070ffdb;'>{$procedimetosRn}</div>  <div style='background:#ffcd05;'>{$alergiasRn}</div> <div style='background:red;'>{$tranfusaoRn}</div></td>
                            <td>{$row['ds_dieta']}</td>
                            <td>{$row['ds_fugulin']}</td>
                            <td>{$row['dt_escalas']}</td>
                            <td>{$row['dt_sae_rn']}</td>
