...
class Validate {
    constructor(formId) {
        ...
        this.formAcessoCatracaEdit = document.getElementById("formAcessoCatracaEdit");
        ...
        if (window.location.pathname.includes('gestaoPessoas.php')) {
            this.validateformAcessoCatraca();
            this.validateformAcessoCatracaEdit();
        }
    }
    ...
    addInputEventListeners() {
        ...
        const formFields = ["patrimonio", "monitor", "descricao", "custo", "local", "hostAntigo", "hostNovo", "login", "office", "observacao", "dataPrev", "dataProx", "tecnico", "perifericos", "andar", "modelo", "sistemaOperacional"];
        ...
        const formFieldsEditar = ["patrimonioEditar", "monitorEditar", "descricaoEditar", "custoEditar", "localEditar", "hostAntigoEditar", "hostNovoEditar", "loginEditar", "officeEditar", "observacaoEditar", "dataPrevEditar", "dataProxEditar", "tecnicoEditar", "perifericosEditar", "andarEditar", "modeloEditar", "sistemaOperacionalEditar"];
    ...
    validateformAcessoCatraca() {
        const formFields = ["numCra", "numFis", "btn-nivel-acesso"];
        document.addEventListener('DOMContentLoaded', function () {
            var btnNivelAcesso = document.getElementById('btn-nivel-acesso');
            btnNivelAcesso.addEventListener('click', function () {
                $('.nivel-acesso').prop('hidden', false);
            });
        });

        formFields.forEach(fieldName => {
            const inputField = this.formAcessoCatraca.elements[fieldName];
            $(inputField).on("input", () => this.handleInputFieldAcessoCatraca(fieldName, inputField));
        });
    }

    handleInputFieldAcessoCatraca(fieldName, inputField) {
        inputField.value = inputField.value.toUpperCase();
        switch (fieldName) {
            case 'numCra':
                $(inputField).mask("100000000000");
                break;
            case 'numFis':
                $(inputField).mask("#############");
                break;
    ...
    validateformAcessoCatracaEdit() {
        const formFields = ["dtAdmEdit", "datnasEdit", "datfimEdit"];

        formFields.forEach(fieldName => {
            const inputField = this.formAcessoCatracaEdit.elements[fieldName];
            $(inputField).on("input", () => this.handleInputFieldAcessoCatracaEdit(fieldName, inputField));
        });
    }

    handleInputFieldAcessoCatracaEdit(fieldName, inputField) {
        inputField.value = inputField.value.toUpperCase();
        switch (fieldName) {
            case 'dtAdmEdit':
                $(inputField).mask("00/00/0000");
